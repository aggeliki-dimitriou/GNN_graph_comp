...
class GNN_model(torch.nn.Module):
    def __init__(self, dims, num_features, training, p, model_type):
        ...
        if model_type == 'gin':
          self.setup_gin(num_features, dims)
    ...
    def conv_pass(self, features, edge_index, batch, type=1):
        ...
        if type == 1:
            ...
            if self.layer_num > 1:
                h1 = F.dropout(h1, p=self.p, training=self.training)
                h1 = self.conv2_1(h1, edge_index)
                h1 = h1.relu()
                ret.append(h1)

            if self.layer_num > 2:
                h1 = F.dropout(h1, p=self.p, training=self.training)
                h1 = self.conv2_2(h1, edge_index)
                h1 = h1.relu()
                ret.append(h1)
    ...
    def forward(self, batch1, batch2):
        ...
        # Node embeddings
        hG1 = self.conv_pass(x1, edge_index1, batch1.batch, 1)
        hG2 = self.conv_pass(x2, edge_index2, batch2.batch, 1)
